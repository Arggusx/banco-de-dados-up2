...
-- QUERY 01
-- ANTES (sem indice funcional, SELECT *, lower() nos dois lados)
...
-- DEPOIS (indice funcional + projecao minima)
...
-- QUERY 02
-- ANTES (subqueries correlacionadas no SELECT + IN com subquery)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT
    v.numero_voo,
...
-- DEPOIS (JOINs diretos + indice em reservas(voo_id) com INCLUDE)
...
-- QUERY 03
-- ANTES (DISTINCT desnecessario + OR repetitivo)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- QUERY 04
-- ANTES (subquery correlacionada que recalcula peso para cada linha)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT
    v.numero_voo,
    ao.codigo_iata,
...
-- DEPOIS (agregacao em fluxo unico + indice covering em bagagens)
...
-- QUERY 05
-- ANTES (SELECT * + to_char no WHERE + ORDER BY com funcao)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT *
...
-- DEPOIS (intervalo SARGable + projecao minima + indice em hora_embarque)
...
-- QUERY 06
-- ANTES (UPPER() no WHERE + OR + sem filtro de ativo)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (valores ja normalizados + IN + indice parcial para ativos)
...
-- DEPOIS (intervalo SARGable + indice em data_reserva)
...
-- ANTES (NOT IN com subquery — risco de NULL + Seq Scan duplo)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT *
FROM funcionarios f
...
-- DEPOIS (NOT EXISTS anti-join + indice parcial)
...
-- ANTES (COALESCE desnecessario + IN com subquery agregada)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (filtro direto + HAVING inline + indice em milhas)
...
-- ANTES (subquery correlacionada COUNT(*) executada por linha)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT
    fab.nome,
    ma.nome,
...
-- DEPOIS (pre-agregacao com CTE + indice em reservas(voo_id, status))
...
-- ANTES (concatenacao + LOWER + LIKE no predicado)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (filtro por coluna indexada + pattern anchored a esquerda)
...
-- ANTES (ILIKE em descricao textual + DISTINCT desnecessario)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (filtro por codigo/ID exato + indice em voos(status_id) + sem DISTINCT)
...
-- ANTES (SUBSTRING no filtro — non-SARGable)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (range scan SARGable + indice em cartoes(terminal_id, gate))
...
-- ANTES (subquery correlacionada COUNT(*) no SELECT e ORDER BY)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (pre-agregacao CTE + indice em voos(aeronave_id))
...
-- ANTES (EXTRACT/EPOCH no predicado — non-SARGable)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (predicado SARGable com INTERVAL + indice composto de rota)
...
-- ANTES (NOT IN — risco NULL + materializa resultado inteiro)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (NOT EXISTS anti-join + indice parcial em modelos)
...
-- ANTES (DISTINCT em agregacao + ORDER BY alias recalculado)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (pre-agregacao por destino + JOIN dimensional + sem DISTINCT)
...
-- ANTES (JOINs completos antes de agregar — cardinalidade explode)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
SELECT f.id, f.nome, f.sobrenome, c.nome AS cargo,
...
-- DEPOIS (pre-agregacao + JOIN dimensional + indice em escalas)
...
-- ANTES (TO_CHAR no WHERE e GROUP BY — Seq Scan + Sort excessivo)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (range scan + date_trunc + indice em hora_embarque)
...
-- ANTES (CTEs MATERIALIZED impedem pushdown do filtro preco_base)
EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)
...
-- DEPOIS (CTEs filtram apenas voos caros + indice em preco_base)
...
-- Remove indices de benchmark para restaurar baseline
