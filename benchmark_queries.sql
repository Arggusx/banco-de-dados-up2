-- ===================================================================
-- BENCHMARK DE QUERIES — SISTEMA AEROPORTUÁRIO
-- PostgreSQL 14+
-- ===================================================================
-- Contém exatamente 20 operações de consulta complexas com análises
-- executadas diretamente na base de dados com 60.000 registos.
-- ===================================================================


-- ============================================================
-- QUERY 1: Passageiros com voos para um país específico
-- Tabelas envolvidas: passageiros, reservas, voos, aeroportos, cidades, paises
-- Antipadrão demonstrado: SELECT * + Função LOWER no WHERE que desabilita índice funcional se não criado
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • SELECT * + Função LOWER no WHERE que desabilita índice funcional se não criado
-- ────────────────────────────────────────────────────────────────

SELECT * FROM passageiros p
JOIN reservas r ON p.id = r.passageiro_id
...
WHERE LOWER(p.email) = 'ana.souza.1@example.com';

-- EXPLAIN ANALYZE (Antes) — Árvore de Consulta:
-- ┌─────────────────────────────────────────────────────────────┐
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 159.43..305.97
-- Tempo de execução real: 2.587 ms

-- ────────────────────────────────────────────────────────────────
-- ETAPA 2: ESTRATÉGIA DE OTIMIZAÇÃO
-- ────────────────────────────────────────────────────────────────
-- Problema identificado:
--   Falta de índices adequados, uso de funções no WHERE ou subqueries ineficientes.
--
-- Plano de ação:
--   Selecionar apenas colunas necessárias, usar o índice funcional LOWER(email) idx_passageiros_email_lower.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT p.id, p.nome, p.sobrenome, p.email, v.numero_voo, pa.nome as pais_destino
FROM passageiros p
JOIN reservas r ON p.id = r.passageiro_id
JOIN voos v ON v.id = r.voo_id
JOIN aeroportos a ON a.id = v.aeroporto_destino_id
...
-- EXPLAIN ANALYZE (Depois) — Árvore de Consulta:
-- ┌─────────────────────────────────────────────────────────────┐
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 60.30..206.85 (Redução de 32.4%)
-- Tempo de execução real: 0.155 ms


-- ============================================================
-- QUERY 2: Receita total de passagens por rota aérea e categoria de tarifa
-- Tabelas envolvidas: voos, reservas, categorias_tarifa, aeroportos
-- Antipadrão demonstrado: Subquery com IN e subqueries correlacionadas no SELECT
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Subquery com IN e subqueries correlacionadas no SELECT
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, 
       (SELECT a.codigo_iata FROM aeroportos a WHERE a.id = v.aeroporto_origem_id) as origem,
       (SELECT a.codigo_iata FROM aeroportos a WHERE a.id = v.aeroporto_destino_id) as destino,
       SUM(r.preco_total) as receita
FROM voos v
JOIN reservas r ON r.voo_id = v.id
WHERE r.categoria_tarifa_id IN (
    SELECT id FROM categorias_tarifa WHERE multiplicador_preco > 1.2
)
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 307.60..44673.38
-- Tempo de execução real: 12.250 ms
...
--
-- Plano de ação:
--   Reescrever subqueries correlacionadas e IN como JOINs diretos, usando covering index.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, ao.codigo_iata as origem, ad.codigo_iata as destino, SUM(r.preco_total) as receita
FROM voos v
JOIN reservas r ON r.voo_id = v.id
...
WHERE ct.multiplicador_preco > 1.2
GROUP BY v.numero_voo, ao.codigo_iata, ad.codigo_iata;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 733.31..766.50 (Redução de 98.28%)
-- Tempo de execução real: 9.752 ms


-- ============================================================
-- QUERY 3: Tripulação escalada para voos internacionais com origem em países específicos
-- Tabelas envolvidas: funcionarios, escalas_tripulacao, voos, aeroportos, cidades, paises
-- Antipadrão demonstrado: Uso de DISTINCT abusivo + operador OR complexo no WHERE
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de DISTINCT abusivo + operador OR complexo no WHERE
-- ────────────────────────────────────────────────────────────────

SELECT DISTINCT f.nome, f.sobrenome, f.email, v.numero_voo
FROM funcionarios f
...
WHERE po.codigo_iso2 = 'BR' OR po.codigo_iso2 = 'PT';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 45.93..46.05
-- Tempo de execução real: 0.120 ms
...
--
-- Plano de ação:
--   Remover DISTINCT redundante (escala garante unicidade por voo/funcionário), reescrever OR como IN.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT f.nome, f.sobrenome, f.email, v.numero_voo
FROM funcionarios f
JOIN escalas_tripulacao et ON et.funcionario_id = f.id
JOIN voos v ON v.id = et.voo_id
JOIN aeroportos ao ON ao.id = v.aeroporto_origem_id
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 5.70..45.76 (Redução de 0.63%)
-- Tempo de execução real: 0.085 ms


-- ============================================================
-- QUERY 4: Peso total de bagagens despachadas por voo
-- Tabelas envolvidas: bagagens, passagens, reservas, voos, tipos_bagagem
-- Antipadrão demonstrado: Subquery correlacionada no SELECT (problema N+1)
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Subquery correlacionada no SELECT (problema N+1)
-- ────────────────────────────────────────────────────────────────

SELECT v.id, v.numero_voo,
       (SELECT SUM(b.peso_kg) 
        FROM bagagens b 
        JOIN passagens p ON p.id = b.passagem_id
        JOIN reservas r ON r.id = p.reserva_id
        WHERE r.voo_id = v.id) as peso_total
FROM voos v;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 0.00..142823.50
-- Tempo de execução real: 52.546 ms
...
--
-- Plano de ação:
--   Substituir subquery correlacionada por LEFT JOINs explícitos e GROUP BY, usando índice parcial de bagagens.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT v.id, v.numero_voo, COALESCE(SUM(b.peso_kg), 0) as peso_total
FROM voos v
LEFT JOIN reservas r ON r.voo_id = v.id
LEFT JOIN passagens p ON p.reserva_id = r.id
LEFT JOIN bagagens b ON b.passagem_id = p.id
GROUP BY v.id, v.numero_voo;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 644.91..707.41 (Redução de 99.5%)
-- Tempo de execução real: 12.704 ms


-- ============================================================
-- QUERY 5: Cartões de embarque com detalhes de voo e terminal
-- Tabelas envolvidas: cartoes_embarque, passagens, reservas, voos, passageiros, terminais
-- Antipadrão demonstrado: SELECT * + joins implícitos (sintaxe antiga ANSI-89 baseada em vírgula)
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • SELECT * + joins implícitos (sintaxe antiga ANSI-89 baseada em vírgula)
-- ────────────────────────────────────────────────────────────────

SELECT *
FROM cartoes_embarque ce, passagens p, reservas r, voos v, passageiros pas, terminais t
WHERE ce.passagem_id = p.id
  AND p.reserva_id = r.id
  AND r.voo_id = v.id
  AND r.passageiro_id = pas.id
  AND ce.terminal_id = t.id
  AND pas.programa_fidelidade = 'SMILES';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 549.80..675.44
-- Tempo de execução real: 4.878 ms
...
--
-- Plano de ação:
--   Usar sintaxe de JOIN explícita (ANSI-92) e selecionar apenas colunas necessárias.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT pas.nome, pas.sobrenome, v.numero_voo, ce.gate, ce.zona_embarque, t.nome as terminal
FROM cartoes_embarque ce
JOIN passagens p ON ce.passagem_id = p.id
JOIN reservas r ON p.reserva_id = r.id
JOIN voos v ON r.voo_id = v.id
JOIN passageiros pas ON r.passageiro_id = pas.id
JOIN terminais t ON ce.terminal_id = t.id
WHERE pas.programa_fidelidade = 'SMILES';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 549.80..675.44 (Redução de 0.0%)
-- Tempo de execução real: 3.746 ms


-- ============================================================
-- QUERY 6: Fornecedores por país com serviços de catering
-- Tabelas envolvidas: fornecedores, paises, cidades, aeroportos
-- Antipadrão demonstrado: Uso de LIKE '%CATERING%' ineficiente impedindo uso de índices de busca direta
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de LIKE '%CATERING%' ineficiente impedindo uso de índices de busca direta
-- ────────────────────────────────────────────────────────────────

SELECT f.nome, pa.nome as pais, f.tipo_servico, f.data_contrato
FROM fornecedores f
JOIN paises pa ON f.pais_id = pa.id
WHERE f.tipo_servico LIKE '%CATERING%'
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 64.51..64.75
-- Tempo de execução real: 0.597 ms
...
--
-- Plano de ação:
--   Usar comparação exata (= 'CATERING') para aproveitar o índice.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 64.51..64.75 (Redução de 0.0%)
-- Tempo de execução real: 0.480 ms


-- ============================================================
-- QUERY 7: Reservas por voo realizadas em determinado mês
-- Tabelas envolvidas: voos, reservas, aeroportos, status_voo
-- Antipadrão demonstrado: Uso da função EXTRACT no WHERE que invalida o índice na coluna data_partida
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso da função EXTRACT no WHERE que invalida o índice na coluna data_partida
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, a.codigo_iata, COUNT(r.id) as total_reservas
FROM voos v
JOIN reservas r ON r.voo_id = v.id
JOIN aeroportos a ON a.id = v.aeroporto_origem_id
WHERE EXTRACT(MONTH FROM v.data_partida) = 6 
  AND EXTRACT(YEAR FROM v.data_partida) = 2026
GROUP BY v.id, v.numero_voo, a.codigo_iata;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 171.46..171.48
-- Tempo de execução real: 1.953 ms
...
--
-- Plano de ação:
--   Substituir EXTRACT por um filtro de intervalo sargable (>= e <) que usa o índice na data.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
JOIN reservas r ON r.voo_id = v.id
JOIN aeroportos a ON a.id = v.aeroporto_origem_id
WHERE v.data_partida >= '2026-06-01 00:00:00+00' 
  AND v.data_partida < '2026-07-01 00:00:00+00'
GROUP BY v.numero_voo, a.codigo_iata;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 349.71..350.59 (Redução de -104.45%)
-- Tempo de execução real: 1.356 ms


-- ============================================================
-- QUERY 8: Funcionários ativos sem escalas de voo atribuídas
-- Tabelas envolvidas: funcionarios, escalas_tripulacao, voos, cargos_funcionarios, aeroportos
-- Antipadrão demonstrado: Uso de NOT IN com subquery pesada que resulta em Seq Scan e pode falhar com NULLs
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de NOT IN com subquery pesada que resulta em Seq Scan e pode falhar com NULLs
-- ────────────────────────────────────────────────────────────────

SELECT f.nome, f.sobrenome, c.nome as cargo, a.nome as aeroporto
FROM funcionarios f
JOIN cargos_funcionarios c ON f.cargo_id = c.id
JOIN aeroportos a ON f.aeroporto_base_id = a.id
WHERE f.id NOT IN (
    SELECT funcionario_id FROM escalas_tripulacao
) AND f.ativo = TRUE;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 329.50..484.50
-- Tempo de execução real: 4.393 ms
...
--
-- Plano de ação:
--   Usar NOT EXISTS que é mais seguro e permite index scan com idx_funcionarios_ativos.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
WHERE f.ativo = TRUE 
  AND NOT EXISTS (
      SELECT 1 FROM escalas_tripulacao et WHERE et.funcionario_id = f.id
  );
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 394.03..561.35 (Redução de -15.86%)
-- Tempo de execução real: 5.150 ms


-- ============================================================
-- QUERY 9: Passageiros com muitas milhas e mais de uma reserva
-- Tabelas envolvidas: passageiros, reservas, voos, aeroportos, paises
-- Antipadrão demonstrado: Uso de filtro IN redundante no HAVING executado após o agrupamento
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de filtro IN redundante no HAVING executado após o agrupamento
-- ────────────────────────────────────────────────────────────────

SELECT p.nome, p.sobrenome, p.email, COUNT(r.id) as total_reservas
FROM passageiros p
JOIN reservas r ON r.passageiro_id = p.id
JOIN voos v ON r.voo_id = v.id
JOIN aeroportos a ON a.id = v.aeroporto_origem_id
JOIN paises pa ON p.nacionalidade_id = pa.id
GROUP BY p.id, p.nome, p.sobrenome, p.email
HAVING COUNT(r.id) > 1 
   AND p.id IN (SELECT id FROM passageiros WHERE milhas_acumuladas > 5000);
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 910.46..985.46
-- Tempo de execução real: 13.116 ms
...
--
-- Plano de ação:
--   Mover o filtro de milhas do HAVING para a cláusula WHERE, reduzindo as linhas antes do agrupamento.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
JOIN voos v ON r.voo_id = v.id
JOIN aeroportos a ON a.id = v.aeroporto_origem_id
WHERE p.milhas_acumuladas > 5000
GROUP BY p.id, p.nome, p.sobrenome, p.email
HAVING COUNT(r.id) > 1;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 718.27..780.15 (Redução de 20.83%)
-- Tempo de execução real: 10.140 ms


-- ============================================================
-- QUERY 10: Ocupação média por modelo de aeronave e fabricante
-- Tabelas envolvidas: aeronaves, modelos_aeronave, fabricantes, voos, reservas
-- Antipadrão demonstrado: Subquery correlacionada no SELECT com divisão de inteiros truncada
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Subquery correlacionada no SELECT com divisão de inteiros truncada
-- ────────────────────────────────────────────────────────────────

SELECT f.nome as fabricante, ma.nome as modelo,
       AVG( (SELECT COUNT(r.id) FROM reservas r WHERE r.voo_id = v.id) / ma.capacidade_passageiros * 100 ) as ocupacao_media
FROM voos v
JOIN aeronaves a ON v.aeronave_id = a.id
JOIN modelos_aeronave ma ON a.modelo_id = ma.id
JOIN fabricantes f ON ma.fabricante_id = f.id
GROUP BY f.nome, ma.id, ma.nome, ma.capacidade_passageiros;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 679.19..56506.69
-- Tempo de execução real: 48.455 ms
...
--
-- Plano de ação:
--   Usar subquery de agregação prévia (JOIN) e cast explícito para DECIMAL.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
SELECT f.nome as fabricante, ma.nome as modelo,
       ROUND(AVG(counts.total_res::DECIMAL / ma.capacidade_passageiros * 100), 2) as ocupacao_media
FROM (
    SELECT voo_id, COUNT(id) as total_res 
    FROM reservas 
    WHERE status <> 'CANCELADA' 
    GROUP BY voo_id
) counts
JOIN voos v ON v.id = counts.voo_id
JOIN aeronaves a ON v.aeronave_id = a.id
JOIN modelos_aeronave ma ON a.modelo_id = ma.id
JOIN fabricantes f ON ma.fabricante_id = f.id
GROUP BY f.nome, ma.nome, ma.capacidade_passageiros;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 654.45..698.17 (Redução de 98.76%)
-- Tempo de execução real: 9.684 ms


-- ============================================================
-- QUERY 11: Histórico de passagens compradas por um passageiro
-- Tabelas envolvidas: passageiros, reservas, passagens, voos, aeroportos, cartoes_embarque
-- Antipadrão demonstrado: Múltiplas subqueries aninhadas ineficientes (IN dentro de IN)
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Múltiplas subqueries aninhadas ineficientes (IN dentro de IN)
-- ────────────────────────────────────────────────────────────────

SELECT numero_passagem, assento, classe 
FROM passagens 
WHERE reserva_id IN (
    SELECT id FROM reservas WHERE passageiro_id IN (
        SELECT id FROM passageiros WHERE LOWER(email) = 'ana.souza.1@example.com'
    )
);
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 172.67..181.58
-- Tempo de execução real: 0.077 ms
...
--
-- Plano de ação:
--   Substituir subqueries aninhadas por JOINs planos diretos e usar índice de email.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT p.numero_passagem, p.assento, p.classe, v.numero_voo, r.preco_total
FROM passageiros pas
JOIN reservas r ON r.passageiro_id = pas.id
JOIN passagens p ON p.reserva_id = r.id
JOIN voos v ON r.voo_id = v.id
WHERE LOWER(pas.email) = 'ana.souza.1@example.com';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 59.75..190.13 (Redução de -4.71%)
-- Tempo de execução real: 0.064 ms


-- ============================================================
-- QUERY 12: Voos atrasados ou cancelados e impacto nos tripulantes
-- Tabelas envolvidas: voos, status_voo, escalas_tripulacao, funcionarios, aeroportos
-- Antipadrão demonstrado: Uso de cláusula OR complexa no WHERE na filtragem de status
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de cláusula OR complexa no WHERE na filtragem de status
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, sv.codigo as status, f.nome, f.sobrenome
FROM voos v
JOIN status_voo sv ON v.status_id = sv.id
JOIN escalas_tripulacao et ON et.voo_id = v.id
JOIN funcionarios f ON et.funcionario_id = f.id
WHERE sv.codigo = 'ATRASADO' OR sv.codigo = 'CANCELADO';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 13.45..61.34
-- Tempo de execução real: 0.265 ms
...
--
-- Plano de ação:
--   Usar IN para permitir otimizações de índice compostos.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
JOIN escalas_tripulacao et ON et.voo_id = v.id
JOIN funcionarios f ON et.funcionario_id = f.id
WHERE sv.codigo IN ('ATRASADO', 'CANCELADO');
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 13.45..61.34 (Redução de 0.0%)
-- Tempo de execução real: 0.146 ms


-- ============================================================
-- QUERY 13: Receita por terminal e gate
-- Tabelas envolvidas: terminais, cartoes_embarque, passagens, reservas, categorias_tarifa
-- Antipadrão demonstrado: Agrupamento inútil por chaves PK redundantes no GROUP BY
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Agrupamento inútil por chaves PK redundantes no GROUP BY
-- ────────────────────────────────────────────────────────────────

SELECT t.nome as terminal, ce.gate, 
       SUM(r.preco_total) as receita_estimada,
       COUNT(p.id) as total_passagens
FROM terminais t
JOIN cartoes_embarque ce ON ce.terminal_id = t.id
JOIN passagens p ON ce.passagem_id = p.id
JOIN reservas r ON p.reserva_id = r.id
GROUP BY t.nome, ce.gate, t.id, ce.id;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 540.95..603.45
-- Tempo de execução real: 10.754 ms
...
--
-- Plano de ação:
--   Simplificar GROUP BY removendo IDs redundantes, evitando ordenações adicionais na memória.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
SELECT t.nome as terminal, ce.gate, 
       SUM(r.preco_total) as receita_real,
       COUNT(p.id) as total_passagens
FROM terminais t
JOIN cartoes_embarque ce ON ce.terminal_id = t.id
JOIN passagens p ON ce.passagem_id = p.id
JOIN reservas r ON p.reserva_id = r.id
GROUP BY t.nome, ce.gate;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 540.95..603.45 (Redução de 0.0%)
-- Tempo de execução real: 10.169 ms


-- ============================================================
-- QUERY 14: Aeronaves com maior utilização por quilometragem
-- Tabelas envolvidas: aeronaves, modelos_aeronave, fabricantes, voos, reservas
-- Antipadrão demonstrado: Uso de ORDER BY com expressão matemática complexa sem índice
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de ORDER BY com expressão matemática complexa sem índice
-- ────────────────────────────────────────────────────────────────

SELECT a.matricula, ma.nome as modelo, f.nome as fabricante,
       COUNT(v.id) as total_voos, ma.alcance_km as km_estimados
FROM aeronaves a
JOIN modelos_aeronave ma ON a.modelo_id = ma.id
JOIN fabricantes f ON ma.fabricante_id = f.id
JOIN voos v ON v.aeronave_id = a.id
GROUP BY a.id, a.matricula, ma.nome, f.nome, ma.alcance_km
ORDER BY COUNT(v.id) * ma.alcance_km DESC;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 804.19..816.69
-- Tempo de execução real: 9.333 ms
...
--
-- Plano de ação:
--   Pré-agrupar voos em subquery para reduzir número de linhas antes dos JOINs complexos.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
SELECT a.matricula, ma.nome as modelo, f.nome as fabricante,
       v_stats.total_voos, (v_stats.total_voos * ma.alcance_km) as km_estimados
FROM (
    SELECT aeronave_id, COUNT(id) as total_voos
    FROM voos
    GROUP BY aeronave_id
) v_stats
JOIN aeronaves a ON a.id = v_stats.aeronave_id
JOIN modelos_aeronave ma ON a.modelo_id = ma.id
JOIN fabricantes f ON ma.fabricante_id = f.id
ORDER BY km_estimados DESC;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 614.22..622.01 (Redução de 23.84%)
-- Tempo de execução real: 6.192 ms


-- ============================================================
-- QUERY 15: Conexões possíveis entre voos no mesmo terminal com intervalo curto
-- Tabelas envolvidas: voos, aeroportos, terminais, cidades, paises
-- Antipadrão demonstrado: Self-join sem cláusula ON apropriada filtrando apenas no WHERE
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Self-join sem cláusula ON apropriada filtrando apenas no WHERE
-- ────────────────────────────────────────────────────────────────

SELECT v1.numero_voo as voo_chegada, v2.numero_voo as voo_partida,
       a.nome as aeroporto, t.nome as terminal
FROM voos v1
JOIN voos v2 ON v1.aeroporto_destino_id = v2.aeroporto_origem_id
JOIN aeroportos a ON v1.aeroporto_destino_id = a.id
JOIN terminais t ON t.aeroporto_id = a.id
WHERE v1.data_chegada < v2.data_partida 
  AND v2.data_partida - v1.data_chegada < INTERVAL '4 hours';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 199.31..470.15
-- Tempo de execução real: 2.106 ms
...
--
-- Plano de ação:
--   Filtrar a data da conexão diretamente na cláusula ON do JOIN para limitar o plano carteseano.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
JOIN voos v2 ON v2.aeroporto_origem_id = v1.aeroporto_destino_id 
            AND v2.data_partida > v1.data_chegada 
            AND v2.data_partida <= v1.data_chegada + INTERVAL '4 hours';
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 199.31..469.78 (Redução de 0.08%)
-- Tempo de execução real: 2.162 ms


-- ============================================================
-- QUERY 16: Passageiros de voos de longa distância que não despacharam bagagem
-- Tabelas envolvidas: passageiros, passagens, reservas, voos, bagagens
-- Antipadrão demonstrado: Uso de LEFT JOIN com filtro IS NULL ineficiente
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso de LEFT JOIN com filtro IS NULL ineficiente
-- ────────────────────────────────────────────────────────────────

SELECT pas.nome, pas.sobrenome, pas.email, v.numero_voo
FROM passageiros pas
JOIN reservas r ON r.passageiro_id = pas.id
JOIN voos v ON r.voo_id = v.id
JOIN passagens p ON p.reserva_id = r.id
LEFT JOIN bagagens b ON b.passagem_id = p.id
WHERE b.id IS NULL AND v.preco_base > 1500;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 157.35..267.73
-- Tempo de execução real: 11.014 ms
...
--
-- Plano de ação:
--   Usar NOT EXISTS para permitir short-circuiting no escaneamento de registros.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
JOIN voos v ON r.voo_id = v.id
JOIN passagens p ON p.reserva_id = r.id
WHERE v.preco_base > 1500
  AND NOT EXISTS (
      SELECT 1 FROM bagagens b WHERE b.passagem_id = p.id
  );
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 584.91..744.23 (Redução de -177.98%)
-- Tempo de execução real: 5.489 ms


-- ============================================================
-- QUERY 17: Ranking de rotas mais lucrativas por país de destino
-- Tabelas envolvidas: voos, reservas, aeroportos, cidades, categorias_tarifa
-- Antipadrão demonstrado: Window function DENSE_RANK executada em joins planos amplos e ordenação ineficiente
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Window function DENSE_RANK executada em joins planos amplos e ordenação ineficiente
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, pa.nome as pais_destino, SUM(r.preco_total) as receita,
       DENSE_RANK() OVER (ORDER BY SUM(r.preco_total) DESC) as ranking
FROM voos v
JOIN reservas r ON r.voo_id = v.id
JOIN aeroportos a ON v.aeroporto_destino_id = a.id
JOIN cidades c ON a.cidade_id = c.id
JOIN paises pa ON c.pais_id = pa.id
GROUP BY v.numero_voo, pa.nome
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 1367.52..1380.02
-- Tempo de execução real: 15.376 ms
...
--
-- Plano de ação:
--   Agregar os custos e faturamento em subquery antes de juntar dados geográficos adicionais.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT v.numero_voo, pa.nome as pais_destino, v_rec.receita,
       DENSE_RANK() OVER (ORDER BY v_rec.receita DESC) as ranking
FROM (
    SELECT voo_id, SUM(preco_total) as receita
    FROM reservas
    GROUP BY voo_id
) v_rec
JOIN voos v ON v.id = v_rec.voo_id
JOIN aeroportos a ON v.aeroporto_destino_id = a.id
JOIN cidades c ON a.cidade_id = c.id
JOIN paises pa ON c.pais_id = pa.id
ORDER BY ranking ASC;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 1042.67..1050.51 (Redução de 23.88%)
-- Tempo de execução real: 11.848 ms


-- ============================================================
-- QUERY 18: Número de escalas por funcionário, cargo e aeroporto base
-- Tabelas envolvidas: funcionarios, escalas_tripulacao, voos, cargos_funcionarios, aeroportos
-- Antipadrão demonstrado: Agrupamento com excessivas colunas redundantes e Joins executados antes do GROUP BY
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Agrupamento com excessivas colunas redundantes e Joins executados antes do GROUP BY
-- ────────────────────────────────────────────────────────────────

SELECT f.nome, f.sobrenome, c.nome as cargo, a.nome as aeroporto, COUNT(v.id) as total_voos
FROM funcionarios f
JOIN escalas_tripulacao et ON et.funcionario_id = f.id
JOIN voos v ON et.voo_id = v.id
JOIN cargos_funcionarios c ON f.cargo_id = c.id
JOIN aeroportos a ON f.aeroporto_base_id = a.id
GROUP BY f.id, f.nome, f.sobrenome, c.id, c.nome, a.id, a.nome;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 776.59..826.59
-- Tempo de execução real: 12.391 ms
...
--
-- Plano de ação:
--   Pré-agrupar escalas por ID do funcionário, depois executar os JOINs apenas para os IDs agregados.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT f.nome, f.sobrenome, c.nome as cargo, a.nome as aeroporto, et_stats.total_voos
FROM (
    SELECT funcionario_id, COUNT(voo_id) as total_voos
    FROM escalas_tripulacao
    GROUP BY funcionario_id
) et_stats
JOIN funcionarios f ON f.id = et_stats.funcionario_id
JOIN cargos_funcionarios c ON f.cargo_id = c.id
JOIN aeroportos a ON f.aeroporto_base_id = a.id;
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 444.08..603.75 (Redução de 26.96%)
-- Tempo de execução real: 7.010 ms


-- ============================================================
-- QUERY 19: Embarques realizados agrupados por terminal e mês
-- Tabelas envolvidas: cartoes_embarque, passagens, reservas, voos, terminais, aeroportos
-- Antipadrão demonstrado: Uso indevido da função TO_CHAR no WHERE e GROUP BY, impedindo o uso de índices de data
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso indevido da função TO_CHAR no WHERE e GROUP BY, impedindo o uso de índices de data
-- ────────────────────────────────────────────────────────────────

SELECT t.nome as terminal, TO_CHAR(ce.hora_embarque, 'YYYY-MM') as mes, COUNT(ce.id) as total_embarques
FROM cartoes_embarque ce
JOIN terminais t ON ce.terminal_id = t.id
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 158.21..158.77
-- Tempo de execução real: 8.547 ms
...
--
-- Plano de ação:
--   Utilizar comparação por faixa de datas sargable (>= e <) no WHERE e DATE_TRUNC no GROUP BY.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────

SELECT t.nome as terminal, DATE_TRUNC('month', ce.hora_embarque) as mes, COUNT(ce.id) as total_embarques
FROM cartoes_embarque ce
JOIN terminais t ON ce.terminal_id = t.id
WHERE ce.hora_embarque >= '2026-01-01 00:00:00+00' 
  AND ce.hora_embarque < '2027-01-01 00:00:00+00'
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 184.51..211.24 (Redução de -33.05%)
-- Tempo de execução real: 1.920 ms


-- ============================================================
-- QUERY 20: Dashboard executivo consolidado (Faturamento, Total de Bagagens) de voos caros
-- Tabelas envolvidas: voos, aeroportos, reservas, passagens, bagagens
-- Antipadrão demonstrado: Uso indevido da palavra-chave MATERIALIZED em CTEs (impede Inline CTE)
-- ============================================================

-- ────────────────────────────────────────────────────────────────
-- ETAPA 1: Versão NÃO OTIMIZADA (Antes)
-- Antipadrões utilizados:
--   • Uso indevido da palavra-chave MATERIALIZED em CTEs (impede Inline CTE)
-- ────────────────────────────────────────────────────────────────
...
FROM voos v
JOIN aeroportos a ON v.aeroporto_origem_id = a.id
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 1056.71..1140.43
-- Tempo de execução real: 12.901 ms
...
--
-- Plano de ação:
--   Remover MATERIALIZED permitindo que o planejador de consultas faça inline e aplique pushdown do filtro.
--
-- ────────────────────────────────────────────────────────────────
-- ETAPA 3: Versão OTIMIZADA (Depois)
-- ────────────────────────────────────────────────────────────────
...
-- └─────────────────────────────────────────────────────────────┘
-- Custo total estimado: 986.11..1077.67 (Redução de 5.5%)
-- Tempo de execução real: 12.180 ms
